...
Usuario dice algo
    ...
    |       → Ejecutar skill ADD-LOGIN (auth con backend propio)
    |
    ...
    |       → Deploy al VPS via git push + build en servidor
    |
...
| 2 | `add-login` | Auth completa con backend propio |
...
| Backend | Estructura propia (migracion desde GitHub) |
| AI Engine | OpenRouter (si se necesita IA) |
| Validacion | Zod |
| Estado | Zustand |
| Testing | Playwright CLI + MCP |
| Deploy | VPS propio (git push + next build + next start) |
