...
export async function recordAndUploadVideo(
  ...
  // 7. Upload to local storage via server action
  ...
  const filename = `${carouselId}-${Date.now()}.${ext}`
  const folder = `videos/${carouselId}`
  const result = await uploadFileFromClient(base64, folder, filename, mimeType)

  if (result.error) throw new Error(`Error subiendo video: ${result.error}`)
