...
export async function generateCoverImage(
  ...
  if (googleKey) {
    const result = await generateWithImagen3(googleKey, topic, visualSuggestion, carouselId, persist)
    if ('url' in result) return result
  }

  const pexelsKey = process.env.PEXELS_API_KEY
  if (!pexelsKey) return { error: 'No hay motor de imágenes configurado' }
  return searchPexels(pexelsKey, buildSearchQuery(topic, visualSuggestion), carouselId, persist)
}
...
async function searchPexels(
  ...
  const publicUrl = await uploadFile(buffer, 'covers', `${carouselId}-${Date.now()}.jpg`)
