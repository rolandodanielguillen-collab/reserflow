...
interface BrandSettingsFormProps {
  initial: Partial<BrandSettingsValues> | null
}

export function BrandSettingsForm({ initial }: BrandSettingsFormProps) {
  const [values, setValues] = useState<BrandSettingsValues>({
    brandName: initial?.brandName ?? 'RESER+',
    brandTagline: initial?.brandTagline ?? '',
    primaryColor: initial?.primaryColor ?? '#1E40AF',
    secondaryColor: initial?.secondaryColor ?? '#F59E0B',
    accentColor: initial?.accentColor ?? '#10B981',
    brandVoice: initial?.brandVoice ?? 'Profesional, cercano, motivador',
    targetAudience: initial?.targetAudience ?? 'Deportistas y administradores de canchas',
    ycloudApiKey: initial?.ycloudApiKey ?? '',
  ...
  async function handleSubmit(e: React.FormEvent) {
    ...
    setSaveError('')
    const result = await saveBrandSettings({ ...values, logoUrl })
    if (result.error) {
      ...
      <section className="bg-white rounded-xl border border-gray-200 p-6 space-y-4">
        ...
        <div className="flex items-start gap-6">
          ...
          <div className="flex flex-col gap-2 flex-shrink-0">
            {[
              { label: 'Primario', key: 'primaryColor' as const },
              { label: 'Secundario', key: 'secondaryColor' as const },
              { label: 'Acento', key: 'accentColor' as const },
            ].map(({ label, key }) => (
      ...
      <section className="bg-white rounded-xl border border-gray-200 p-6 space-y-4">
        ...
        <div className="grid grid-cols-1 sm:grid-cols-2 gap-4">
          <Field label="Nombre de marca *">
            <input type="text" value={values.brandName} onChange={e => set('brandName', e.target.value)} required className={INPUT} />
          </Field>
          <Field label="Tagline">
            <input type="text" value={values.brandTagline ?? ''} onChange={e => set('brandTagline', e.target.value)} placeholder="Tu slogan" className={INPUT} />
          </Field>
        </div>

        <div className="grid grid-cols-3 gap-4">
          {([
            { label: 'Color primario', key: 'primaryColor' as const },
            { label: 'Color secundario', key: 'secondaryColor' as const },
            { label: 'Color acento', key: 'accentColor' as const },
          ]).map(({ label, key }) => (
        ...
        <div className="grid grid-cols-1 sm:grid-cols-2 gap-4">
          <Field label="Voz de marca">
            <input type="text" value={values.brandVoice ?? ''} onChange={e => set('brandVoice', e.target.value)} placeholder="Profesional, cercano, motivador" className={INPUT} />
          </Field>
          <Field label="Audiencia objetivo">
            <input type="text" value={values.targetAudience ?? ''} onChange={e => set('targetAudience', e.target.value)} placeholder="Deportistas y admins de canchas" className={INPUT} />
          </Field>
        </div>
      ...
      <section className="bg-white rounded-xl border border-gray-200 p-6 space-y-4">
        ...
        <div className="grid grid-cols-1 sm:grid-cols-2 gap-4">
          <Field label="Instagram Account ID">
            <input type="text" value={values.instagramAccountId ?? ''} onChange={e => set('instagramAccountId', e.target.value)} placeholder="17841400000000000" className={INPUT} />
          </Field>
          <Field label="Meta Access Token">
            <input type="password" value={values.metaAccessToken ?? ''} onChange={e => set('metaAccessToken', e.target.value)} placeholder="EAABsbCS..." className={INPUT} />
          </Field>
        </div>
      </section>
      ...
      <section className="bg-white rounded-xl border border-gray-200 p-6 space-y-4">
        ...
        <div className="grid grid-cols-1 sm:grid-cols-2 gap-4">
          <Field label="YCloud API Key">
            <input type="password" value={values.ycloudApiKey ?? ''} onChange={e => set('ycloudApiKey', e.target.value)} placeholder="sk_live_..." className={INPUT} />
          </Field>
          <Field label="Número WhatsApp">
            <input type="text" value={values.whatsappPhone ?? ''} onChange={e => set('whatsappPhone', e.target.value)} placeholder="+5491100000000" className={INPUT} />
          </Field>
        </div>
      </section>
      ...
      <div className="sticky bottom-0 bg-white border-t border-gray-200 -mx-6 px-6 py-4 flex items-center justify-between rounded-b-xl">
        ...
        <button
          ...
          style={{
            ...
            color: isLightColor(values.primaryColor) ? '#111827' : '#ffffff',
            borderColor: isLightColor(values.primaryColor) ? '#D1D5DB' : values.primaryColor,
          }}
        >
