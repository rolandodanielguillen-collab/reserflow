...
export function ContentCalendar() {
  ...
  function getPostsForDay(day: Date) {
    ...
    return posts.filter(p => p.scheduledAt && argDateStr(p.scheduledAt) === dayStr)
  }
    ...
    <div className="space-y-4">
      ...
      {selected && (
        <div className="bg-white rounded-xl border border-gray-200 p-5 space-y-3">
            ...
            <div>
              ...
              {selected.scheduledAt && (
                <p className="text-xs text-gray-400 mt-0.5">
                  {new Date(selected.scheduledAt).toLocaleString('es-AR', { day: 'numeric', month: 'long', hour: '2-digit', minute: '2-digit', timeZone: ARG_TZ })} hs
                </p>
              )}
            </div>
          ...
          {selected.instagramPermalink && (
            <a
              href={selected.instagramPermalink}
              target="_blank"
