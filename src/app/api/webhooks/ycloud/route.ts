...
async function handleApprovalReply(from: string, bodyText: string) {
  ...
  let brand = await prismaAdmin.brandSettings.findFirst({
    where: { whatsappPhone: from },
    select: { userId: true, pendingApprovalCarouselId: true },
  })

  if (!brand) {
    const stripped = from.replace(/^\+/, "")
    brand = await prismaAdmin.brandSettings.findFirst({
      where: { whatsappPhone: { contains: stripped.slice(-10) } },
      select: { userId: true, pendingApprovalCarouselId: true },
    })
  }

  if (!brand?.pendingApprovalCarouselId) return false

  const newStatus = isApproval ? "approved" : "review"

  await prismaAdmin.carousel.updateMany({
    where: { id: brand.pendingApprovalCarouselId, userId: brand.userId },
    data: { status: newStatus },
  })

  await prismaAdmin.brandSettings.update({
    where: { userId: brand.userId },
    data: { pendingApprovalCarouselId: null },
  })

  console.log(`[YCloud] Carrusel ${brand.pendingApprovalCarouselId} → ${newStatus}`)
  return true
}
